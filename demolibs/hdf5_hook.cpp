...
void writeFieldsToH5(char* filename, const std::vector<std::string> fnames, const std::vector<uint> fids, RegionOfFields rgn, BaseGridConfig g_config, Context ctx, Runtime* rt) {

    char h5_file[256];
    sprintf(h5_file, "%s.h5", filename);
    ...
    att_launcher.attach_hdf5(h5_file, field_map, LEGION_FILE_READ_WRITE);
...
void writeFieldsToH5(const std::string filename, const std::vector<std::string> fnames, const std::vector<uint> fids, RegionOfFields rgn, BaseGridConfig g_config, Context ctx, Runtime* rt) {
    ...
    char h5_file[256];
    sprintf(h5_file, "%s.h5", filename.c_str());
